function obj = scgAortic(obj, varargin)

% -------------------------------------------------------------------------
% Description:
% This function extracts the rAO/C interval from SCG waveforms. The feature
% extraction method is optimized for SCG waveforms that are assumed to be
...
%
% Returns:
% - obj.dataset{subject}.(level).scgRAO and/or
% - obj.dataset{subject}.(level).scgRAC
% -------------------------------------------------------------------------
...
for subject = 1:length(subjects)
    ...
    for level = levels
        ...
        if level == Level.relBaseline1 || level == Level.relBaseline2 || ...
            ...
            rao = obj.dataset{subject}.allRelative.scgRAO(beats(1):beats(end));
            rac = obj.dataset{subject}.allRelative.scgRAC(beats(1):beats(end));
            obj.dataset{subject}.(string(level)).scgRAO = rao;
            obj.dataset{subject}.(string(level)).scgRAC = rac;
            continue
        end
        ...
        if level == Level.absBaseline1 || level == Level.absBaseline2 || ...
            ...
            rao = obj.dataset{subject}.allAbsolute.scgRAO(beats(1):beats(end));
            rac = obj.dataset{subject}.allAbsolute.scgRAC(beats(1):beats(end));
            obj.dataset{subject}.(string(level)).scgRAO = rao;
            obj.dataset{subject}.(string(level)).scgRAC = rac;
            continue
        end
        
        % Get the SCG data (front and back split)
        scg_front = obj.dataset{subject}.(string(level)).sternumSCG_z;
        scg_back = obj.dataset{subject}.(string(level)).sternumSCG_z_back;
        ...
        if (AO || both) && level == Level.allAbsolute
            ...
            switch obj.subjects(subject)
                ...
                case 1
                    ...
                    % Subject 1
                    rao = cardio.scg.simpleFeatures(scg_front, 'min', 'from', 100, 'to', 200);
                    
                case 2
                    
                    % Subject 2
                    rao = cardio.scg.simpleFeatures(scg_front, 'max', 'to', 100);
                    
                case 3
                    
                    % Subject 3
                    rao = cardio.scg.simpleFeatures(scg_front, 'max', 'from', 150, 'to', 200);
                    
                case 4
                    
                    % Subject 4
                    rao = cardio.scg.simpleFeatures(scg_front, 'max', 'from', 50, 'to', 150);
                    
                case 5
                    
                    % Subject 5
                    rao = cardio.scg.simpleFeatures(scg_front, 'max');
        ...
        if (AC || both) && level == Level.allAbsolute
            ...
            switch obj.subjects(subject)
                case 1
                    ...
                    % Subject 1
                    rac = cardio.scg.simpleFeatures(scg_front, 'max', 'from', 700, 'to', 800);
                    
                case 2
                    
                    % Subject 2
                    rac = cardio.scg.simpleFeatures(scg_front, 'max', 'from', 750, 'to', 850, 'memory', 15);
                    
                case 3
                    
                    % Subject 3
                    rac = cardio.general.pulseTime(scg_back, 'globalMax', 'min', 100, 'max', 400, 'memory', 30);
                    rac = size(scg_back, 1) - rac;
                ...
                case 4
                    ...
                    % Subject 4
                    rac = cardio.scg.simpleFeatures(scg_front, 'max', 'from', 800, 'to', 1000);
                    
                case 5
                    
                    % Subject 5
                    rac = cardio.scg.simpleFeatures(scg_front, 'min', 'from', 850, 'to', 950);
                    
                case 6
                    
                    % Subject 6
                    rac = cardio.scg.simpleFeatures(scg_front, 'max', 'from', 750, 'to', 850);
        ...
        if (AO || both) && level == Level.allRelative
            ...
            switch obj.subjects(subject)
                ...
                case 1
                    ...
                    % Subject 1
                    rao = cardio.scg.simpleFeatures(scg_front, 'from', 125, 'to', 225);
                    
                case 2
                    
                    % Subject 2
                    rao = cardio.scg.simpleFeatures(scg_front, 'to', 125);
                    
                case 3
                    
                    % Error correction
                    scg_front = scg_front(:, 1:end-1);
        ...
        if (AC || both) && level == Level.allRelative
            ...
            switch obj.subjects(subject)
                ...
                case 2
                    ...
                    % Subject 2
                    rac = cardio.scg.simpleFeatures(scg_front, 'max', 'from', 800, 'to', 900);
                    
                case 3
                    
                    % Error correction
                    scg_back = scg_back(:, 1:end-1);
                    
                    % Subject 3
                    rac = cardio.scg.simpleFeatures(scg_back, 'max', 'from', 250, 'to', 350);
                    rac = size(scg_back, 1) - rac;
                ...
                case 4
                    ...
                    % Subject 4
                    rac = cardio.scg.simpleFeatures(scg_back, 'max', 'from', 100, 'to', 200);
                    rac = size(scg_back, 1) - rac;
                ...
                case 5
                    ...
                    % Subject 5
                    rac = cardio.scg.simpleFeatures(scg_back, 'min', 'from', 125, 'to', 175, 'memory', 11);
                    rac = size(scg_back, 1) - rac;
        ...
        % Save output
        if AO || both; obj.dataset{subject}.(string(level)).scgRAO = rao; end
        if AC || both; obj.dataset{subject}.(string(level)).scgRAC = rac; end
