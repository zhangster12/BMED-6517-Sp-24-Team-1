...
# Assuming 'data_all_1_array' is your dataset as a NumPy array
# 'data_y_all_1_array' contains labels for each feature

# Create a list of feature pairs from the first 5 features
feature_pairs = [(i, j) for i in range(5) for j in range(i + 1, 5)]
...
# Iterate over each pair of features to create scatter plots
for idx, pair in enumerate(feature_pairs):
    feature1, feature2 = pair
    row = idx // num_cols
    col = idx % num_cols

    # For each feature pair, use the corresponding labels
    label_feature1 = data_y_all_1_array[:, feature1]
    label_feature2 = data_y_all_1_array[:, feature2]

    # Define color map based on the labels for the current feature pair
    colors = np.where((label_feature1 == 0) & (label_feature2 == 0), 'blue',
                      np.where((label_feature1 == 1) & (label_feature2 == 1), 'red', 'green'))

    # Scatter plot with colors based on the combined labels of the feature pair
    axes[row, col].scatter(data_all_1_array[:, feature1], data_all_1_array[:, feature2], c=colors, alpha=0.5)
